...
public abstract class JPAModel<M extends JPAModel<M>> extends Model<M> {
  ...
  private static final ThreadLocal<StringBuffer> sqlLocal = new ThreadLocal<StringBuffer>();
  private static final ThreadLocal<List<Object>> valueLocal = new ThreadLocal<List<Object>>();
  ...
  public boolean save() {
    ...
    this.set(table.getPrimaryKey()[0], UUID.randomUUID().toString().replace("-", ""));
...
/*  @Override
  ...
  public M exist() {
    ...
    sqlLocal.set(null);
    valueLocal.set(null);
    boolean exist = Db.find(sql.toString(), args.toArray()).get(0).getLong("count(1)") > 0;
    if (exist)
      return (M) this;
    else
      return null;
  ...
  public Long count() {
    ...
    sqlLocal.set(null);
    valueLocal.set(null);
    return Db.find(sql.toString(), args.toArray()).get(0).getLong("count(1)");
  }

  @SuppressWarnings("unchecked")
  public M groupBy(String name) {
    StringBuffer sql = sqlLocal.get();
    if (sql == null) throw new Error("Oop! must use method 'select()' first.");
    sql.append(" group by ").append(name);
    return (M) this;
  }

  @SuppressWarnings("unchecked")
  public M haveing(String content) {
    StringBuffer sql = sqlLocal.get();
    if (sql == null) throw new Error("Oop! must use method 'select()' first.");
  ...
  public List<M> fetch(int start, int count) {
    StringBuffer sql = sqlLocal.get();
    List<Object> args = valueLocal.get();
    if (sql == null) {
      throw new Error("Oop! must use method 'select()' first.");
    }
        ...
        if (cascadeUpdate) {
          if (obj == null) {
            ...
            if (cascadeDeleteObject) {
              ...
              int deleteIndex = Db.update("delete from mp_" + masterTableName + "_" + slavesTableName + " where " + masterTableName + "_id = ?", this.getStr("id"));
...
//            throw rpc PersistenceException(String.format("Oop! cascade update-delete obj {%s} fail. index: %d", slavesModel.getClass(), deleteIndex));
        }
      }
      // -----
    }
  ...
  private void _cascadeInsert0(Model<M> slavesModel, String masterTableName, String slavesTableName) {
    ...
    int index = Db.update("insert into mp_" + masterTableName + "_" + slavesTableName + " (" + masterTableName + "_id, " + slavesTableName + "_id) value (?, ?)", this.getStr("id"), slavesModel.getStr("id"));
  ...
  private String getCascadeObjectTableName(Type type) {
    if (ParameterizedType.class.isAssignableFrom(type.getClass())) {
      for (Type t0 : ((ParameterizedType) type).getActualTypeArguments()) {
        try {
          com.demo.common.Table o = Class.forName(t0.toString().substring(6)).getAnnotation(com.demo.common.Table.class);
          return o.tableName();
